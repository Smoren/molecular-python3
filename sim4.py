...
def divide_clusters(clusters: Dict[Tuple[int, int], Set[Atom]], threads, exact=False):
    clusters = list(clusters.items())

    num = len(clusters) // threads
    rem = len(clusters) % threads
    if exact and rem:
    ...
    for i in range(threads):
        ...
        divided.append(clusters[start:end])

    return divided


def process_handler(
    clusters: List[Set[Atom]],
    all_clusters: Dict[Tuple[int, int], Set[Atom]],
    num_threads: int,
    master,
    left,
    right,
...
if __name__ == '__main__':
    ...
    distribution = divide_clusters(clusters, num_threads)

    master_pipes = [mp.Pipe() for _ in range(num_threads)]
    p2p_pipes = [mp.Pipe(False) for _ in range(num_threads)]
