...
def interact_cluster(
    ...
    for i in nb.prange(cluster_atoms.shape[0]):
        ...
        eps = atoms_lj_params[int(atom[A_COL_TYPE]), neighbours[:, A_COL_TYPE].astype(np.int64), 0]
        alpha = atoms_lj_params[int(atom[A_COL_TYPE]), neighbours[:, A_COL_TYPE].astype(np.int64), 1]
        sigma = atoms_lj_params[int(atom[A_COL_TYPE]), neighbours[:, A_COL_TYPE].astype(np.int64), 2]
        ...
        fx_next, fy_next, vx_next, vy_next, v_next, u_next, ek_next \
            = get_verlet_next(atom, rx, ry, delta_t, eps, alpha, sigma, gravity)
